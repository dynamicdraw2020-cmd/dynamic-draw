...
import { CalendarDays, LoaderCircle, Megaphone, Plus, Sparkles, Trash2 } from "lucide-react";
...
export function ContentManager({ notices, events }: { notices: Notice[]; events: EventPost[] }) {
  ...
  async function deleteNotice(notice: Notice) {
    ...
    setLoading(`notice-${notice.id}`);
    try { await jsonRequest(`/api/admin/notices/${notice.id}`, {}, "DELETE"); router.refresh(); }
    catch (error) { window.alert((error as Error).message); }
    finally { setLoading(null); }
  }

  async function deleteEvent(item: EventPost) {
    if (!window.confirm(`이벤트 “${item.title}”을 삭제할까요?`)) return;
    setLoading(`event-${item.id}`);
    try { await jsonRequest(`/api/admin/events/${item.id}`, {}, "DELETE"); router.refresh(); }
    catch (error) { window.alert((error as Error).message); }
    finally { setLoading(null); }
  }
  ...
  return <div className="grid">
    <div className="grid grid-2">
      <form className="panel panel-pad form-grid" onSubmit={createNotice}>
        <div className="flex items-center gap-1"><Megaphone size={19} className="text-gold" /><h2 className="panel-title mb-0">공지 등록</h2></div>
        <div className="field"><label htmlFor="notice-title">제목</label><input className="input" id="notice-title" name="title" maxLength={80} required placeholder="공지 제목" /></div>
        <div className="field"><label htmlFor="notice-body">내용</label><textarea className="textarea notice-editor" id="notice-body" name="body" rows={7} maxLength={2000} required placeholder="공지 내용" /></div>
      ...
      <form className="panel panel-pad form-grid" onSubmit={createEvent}>
        <div className="flex items-center gap-1"><Sparkles size={19} className="text-gold" /><h2 className="panel-title mb-0">이벤트 등록</h2></div>
        <div className="form-row"><div className="field"><label htmlFor="event-title">이벤트명</label><input className="input" id="event-title" name="title" maxLength={80} required placeholder="이벤트명" /></div><div className="field"><label htmlFor="event-slug">주소 코드</label><input className="input" id="event-slug" name="slug" maxLength={80} required placeholder="ticket-event" /></div></div>
        ...
        <div className="field"><label htmlFor="event-body">상세 설명</label><textarea className="textarea notice-editor" id="event-body" name="body" rows={5} maxLength={4000} placeholder="참여 방법, 교환 기준, 유의사항" /></div>
    ...
    <div className="grid grid-2">
      <section className="panel panel-pad"><h2 className="panel-title">공지 현황</h2><div className="table-wrap mt-3"><table className="table"><thead><tr><th>제목</th><th>상태</th><th>등록일</th><th>관리</th></tr></thead><tbody>{notices.length ? notices.map((notice) => <tr key={notice.id}><td><strong>{notice.title}</strong><div className="notice-preview text-muted text-small">{notice.body}</div></td><td>{notice.is_public ? "공개" : "숨김"}{notice.is_pinned ? " · 고정" : ""}</td><td className="muted">{formatDateTime(notice.created_at)}</td><td><button className="btn btn-danger btn-sm" type="button" disabled={loading === `notice-${notice.id}`} onClick={() => deleteNotice(notice)}><Trash2 size={13} /> 삭제</button></td></tr>) : <tr><td colSpan={4}><div className="empty">등록된 공지가 없습니다.</div></td></tr>}</tbody></table></div></section>
      <section className="panel panel-pad"><h2 className="panel-title">이벤트 현황</h2><div className="table-wrap mt-3"><table className="table"><thead><tr><th>이벤트</th><th>상태</th><th>기간</th><th>관리</th></tr></thead><tbody>{events.length ? events.map((item) => <tr key={item.id}><td><strong>{item.title}</strong><div className="text-muted text-small">/{item.slug}</div></td><td>{item.status}{item.is_public ? " · 공개" : " · 숨김"}</td><td className="muted"><CalendarDays size={13} style={{ verticalAlign: -2 }} /> {item.starts_at ? formatDateTime(item.starts_at) : "상시"}{item.ends_at ? ` ~ ${formatDateTime(item.ends_at)}` : ""}</td><td><button className="btn btn-danger btn-sm" type="button" disabled={loading === `event-${item.id}`} onClick={() => deleteEvent(item)}><Trash2 size={13} /> 삭제</button></td></tr>) : <tr><td colSpan={4}><div className="empty">등록된 이벤트가 없습니다.</div></td></tr>}</tbody></table></div></section>
    </div>
  </div>;
}
