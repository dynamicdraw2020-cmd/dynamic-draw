...
export function SpinConsole({ draws, members, balances }: { draws: Draw[]; members: Profile[]; balances: AdminTicketBalance[] }) {
  ...
  async function spin() {
    ...
    window.setTimeout(() => { void (async () => { try { const revealResponse = await fetch(`/api/admin/results/${resultId}/reveal`, { method: "POST", cache: "no-store" }); const revealBody = await revealResponse.json(); setLoading(false); if (!revealResponse.ok) { setMessage(revealBody.error?.message ?? "결과는 결정됐지만 공개 처리에 실패했습니다."); return; } setMessage(`결과 공개 완료: ${revealBody.data?.rewardName ?? "상품"}`); } catch (error) { setLoading(false); setMessage(error instanceof Error ? error.message : "결과 공개 중 오류가 발생했습니다."); } })(); }, Number(animationMs) + 100);
  }
