...
type PostRow = { id: string; title: string; body: string; nickname: string | null; created_at: string; community_reports?: Array<{ id: string }> | null };

export default async function AdminCommunityPage() {
  await requireAdmin("MANAGER");
  const { data } = await createAdminClient().from("community_posts").select("id,title,body,nickname,created_at,status,community_reports(id)").order("created_at", { ascending: false }).limit(100);
