...
export default async function AdminGrowthPage() {
  ...
  const [members, draws, levels, drawExp, vipTiers, badges, profileBadges, growthRows, expLogs] = await Promise.all([
  ...
  return <main><div className="page-heading"><h1>레벨·VIP·배지</h1><p>뽑기 경험치, 레벨, VIP 등급, 배지와 휘장을 한곳에서 관리합니다.</p></div><GrowthManager data={{ members: members.data ?? [], draws: draws.data ?? [], levels: levels.data ?? [], drawExp: drawExp.data ?? [], vipTiers: vipTiers.data ?? [], badges: badges.data ?? [], profileBadges: profileBadges.data ?? [], growthRows: growthRows.data ?? [], expLogs: expLogs.data ?? [] }} /></main>;
}
