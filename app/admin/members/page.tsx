...
export default async function MembersPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  const members = allMembers.filter((member) => {
    const matchesText = !query || [member.display_name, member.email, member.phone ?? "", member.member_code ?? ""].some((value) => value.toLowerCase().includes(query));
  ...
  return <>
    ...
    <form className="panel panel-pad form-grid mt-3" method="get">
      <div className="form-row">
        <div className="field"><label htmlFor="member-query">회원 검색</label><input className="input" id="member-query" name="q" defaultValue={first(params.q)} placeholder="이름, 이메일, 연락처, 고유 ID" /></div>
