...
export default async function AdminSettingsPage() {
  ...
  const initial = { siteName: "𝐃𝐲𝐧𝐚𝐦𝐢𝐜 𝐃", heroTitle: "𝐃𝐲𝐧𝐚𝐦𝐢𝐜 𝐃", heroDescription: "𝐃𝐲𝐧𝐚𝐦𝐢𝐜 Event server", publicStats: true, operationMode: "NORMAL", operationMessage: "현재 시스템 점검 중입니다.", operationEndsAt: "", footerMessage: "𝐃𝐲𝐧𝐚𝐦𝐢𝐜 전용 이벤트 운영 사이트 · v1.0.3", monthlyRankImageUrl: "" };
  if (!demoMode) {
    const admin = createAdminClient();
    const { data } = await admin.from("site_settings").select("key,value").in("key", ["site_name", "hero_title", "hero_description", "public_stats", "operation_mode", "operation_message", "operation_ends_at", "footer_message", "monthly_rank_image_url"]);
