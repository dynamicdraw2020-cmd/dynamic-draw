...
export default async function AdminSupportPage() {
  ...
  const { data } = await createAdminClient().from("support_tickets").select("id,category,title,body,status,admin_reply,attachments,created_at,updated_at,profiles(display_name,username)").order("created_at", { ascending: false }).limit(120);
