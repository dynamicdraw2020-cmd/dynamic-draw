...
const schema = z.object({
  confirm: z.literal("RESET_ALL_PASSWORDS"),
...
async function postHandler(request: Request) {
  ...
  if (!parsed.success) return fail("일괄 초기화를 진행하려면 RESET_ALL_PASSWORDS 확인값이 필요합니다.", 422, "VALIDATION_ERROR");
  ...
  if (!parsed.data.includeAdmins) query = query.neq("role", "SUPER_ADMIN").neq("role", "MANAGER").neq("role", "CS_MANAGER");
  ...
  for (const target of (targets ?? []) as Target[]) {
    try {
      ...
      if (!email) {
        failed.push({ ...target, reason: "profile email is empty" });
        continue;
      }
      const { error } = await admin.auth.admin.updateUserById(target.id, { email, password: TEMPORARY_PASSWORD, email_confirm: true });
      if (error) {
  ...
  return ok({
    ...
    hasMore: (targets ?? []).length >= parsed.data.limit,
    message: `초기화 완료: ${succeeded.length}명 성공, ${failed.length}명 실패`,
  });
}

export const POST = withApiRoute(postHandler, { routeName: "/api/admin/members/bulk-reset-passwords", rateLimit: { kind: "admin", limit: 5, windowSeconds: 60 } });
