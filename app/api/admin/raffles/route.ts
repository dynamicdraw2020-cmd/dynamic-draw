...
export async function POST(request: Request) {
  ...
  const parsed = schema.safeParse(await readJsonWithLimit(request).catch(() => null)); if (!parsed.success) return fail(parsed.error.issues[0]?.message ?? "추첨 이벤트 정보를 확인해 주세요.", 422, "VALIDATION_ERROR");
