...
export async function PATCH(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const { id } = await context.params; const parsed = patchSchema.safeParse(await readJsonWithLimit(request).catch(() => null)); if (!parsed.success) return fail("교환비를 확인해 주세요.", 422);
