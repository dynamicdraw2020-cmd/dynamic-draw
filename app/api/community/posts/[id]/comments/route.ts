...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const { data, error } = await admin.from("community_comments").insert({ post_id: id, profile_id: guard.auth.userId, body: parsed.data.body, nickname: parsed.data.nickname || guard.auth.profile.display_name }).select("*").single();
