...
export default async function HomePage() {
  ...
  return <main className="public-home community-home"><RealtimeRefresh />
    ...
    <section className="community-hero"><div className="container community-hero-grid"><div className="community-copy"><span className="section-kicker"><ShieldCheck size={14} /> Official Dynamic Event</span><h1>𝐃𝐲𝐧𝐚𝐦𝐢𝐜 𝐃 - 이벤트 전용 사이트</h1><p className="official-lead">𝐃𝐲𝐧𝐚𝐦𝐢𝐜에서 주관하는 모든 뽑기(추첨)형 이벤트를 주관하는 사이트.</p><p className="official-sublead">𝐃𝐲𝐧𝐚𝐦𝐢𝐜 𝐃 - 누구보다 빠른 본방 입성을 향한 길.</p><div className="official-actions"><Link className="btn btn-primary btn-lg" href="/events"><Ticket size={18} /> 이벤트 확인</Link><Link className="btn btn-secondary btn-lg" href="/notices"><Megaphone size={18} /> 공지 보기</Link><Link className="btn btn-secondary btn-lg" href="/raffles"><UsersRound size={18} /> 전체 추첨</Link></div><div className="safe-service-note"><CheckCircle2 size={16} /> 결과·지급·교환 기록을 공개 기준에 맞춰 보존하는 이벤트 운영 서비스입니다.</div></div><aside className="official-contact-card clean-contact-card"><h2>공식 채널</h2><p>공지, 참여 안내, 문의는 아래 공식 채널을 기준으로 운영됩니다.</p><div className="official-link-list">{officialLinks.map((item) => <a key={item.label} href={item.href} target={item.href === "#" ? undefined : "_blank"} rel="noreferrer"><span>{item.label}</span><strong>{item.text ?? item.href}</strong><ExternalLink size={15} /></a>)}</div></aside></div></section>
    ...
    <section className="official-section compact-official-section"><div className="container public-card policy-card"><ShieldCheck size={22} /><div><h2>전체 이용가 운영 톤</h2><p>𝐃𝐲𝐧𝐚𝐦𝐢𝐜 𝐃는 실제 결제나 배팅을 다루지 않는 이벤트 안내·추첨 운영 사이트입니다. 개인정보는 아이디와 닉네임 중심으로 최소 수집합니다.</p></div></div></section>
  </main>;
}
