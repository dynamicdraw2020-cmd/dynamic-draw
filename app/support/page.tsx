...
export default async function SupportPage() {
  ...
  const { data } = await createAdminClient().from("support_tickets").select("id,category,title,body,status,admin_reply,created_at,updated_at").eq("profile_id", profile.id).order("created_at", { ascending: false }).limit(50);
