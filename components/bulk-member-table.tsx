...
export function BulkMemberTable({ members, currentAdmin }: { members: Profile[]; currentAdmin: Profile }) {
  ...
  async function suspendAllRegularMembers() {
    ...
    const confirmText = window.prompt(`관리자 제외 승인 일반 회원 ${count.toLocaleString()}명을 모두 이용정지합니다. 계속하려면 SUSPEND를 입력해 주세요.`);
    if (confirmText !== "SUSPEND") return;
    const reason = window.prompt("이용정지 사유를 입력해 주세요.")?.trim();
    if (!reason) return;
    setBulkSuspending(true);
    try {
      const response = await fetch("/api/admin/members/bulk-suspend", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ reason }) });
      const body = await response.json();
      if (!response.ok) throw new Error(body.error?.message ?? "전체 이용정지에 실패했습니다.");
  ...
  async function restoreAllRegularMembers() {
    ...
    const confirmText = window.prompt(`관리자 제외 이용정지 일반 회원 ${count.toLocaleString()}명을 승인 상태로 복구합니다. 계속하려면 RESTORE를 입력해 주세요.`);
    if (confirmText !== "RESTORE") return;
    setBulkRestoring(true);
    try {
      const response = await fetch("/api/admin/members/bulk-restore", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ reason: "전체 이용정지 해지" }) });
      const body = await response.json();
      if (!response.ok) throw new Error(body.error?.message ?? "전체 이용정지 해지에 실패했습니다.");
  ...
  return <>
    <section className="panel bulk-member-bar">
      ...
      <div className="table-actions">
        ...
        <button className="btn btn-success" type="button" onClick={approveSelected} disabled={loading || !selectedPending.length}>{loading ? <LoaderCircle size={16} className="spin" /> : <CheckSquare size={16} />} 선택 {selectedPending.length}명 승인</button>
        {currentAdmin.role === "SUPER_ADMIN" && <button className="btn btn-secondary" type="button" onClick={restoreAllRegularMembers} disabled={bulkRestoring}>{bulkRestoring ? <LoaderCircle size={16} className="spin" /> : <RotateCcw size={16} />} 일반 회원 정지 해지</button>}
        {currentAdmin.role === "SUPER_ADMIN" && <button className="btn btn-danger" type="button" onClick={suspendAllRegularMembers} disabled={bulkSuspending}>{bulkSuspending ? <LoaderCircle size={16} className="spin" /> : <ShieldBan size={16} />} 일반 회원 전체 정지</button>}
        {currentAdmin.role === "SUPER_ADMIN" && <button className="btn btn-danger" type="button" onClick={deleteNonSuperAdmins} disabled={bulkDeleting}>{bulkDeleting ? <LoaderCircle size={16} className="spin" /> : <Trash2 size={16} />} 총 관리자 제외 전체 삭제</button>}
      </div>
    </section>
    <div className="table-wrap mt-3"><table className="table"><thead><tr><th style={{ width: 46 }}><input type="checkbox" checked={allSelected} onChange={toggleAll} aria-label="승인 대기 회원 전체 선택" /></th><th>회원</th><th>아이디</th><th>고유 ID</th><th>상태</th><th>접속/위험</th><th>권한</th><th>신청일</th><th>처리</th></tr></thead><tbody>{members.length ? members.map((member) => {
      const canBulkSelect = member.status === "PENDING" && member.role === "USER" && member.id !== currentAdmin.id;
      return <tr key={member.id}><td><input type="checkbox" disabled={!canBulkSelect} checked={selected.includes(member.id)} onChange={() => toggle(member.id)} aria-label={`${member.display_name} 선택`} /></td><td><strong>{member.display_name}</strong><div className="text-muted text-small">개인정보 최소 수집 계정</div></td><td className="muted">{displayLoginId(member)}</td><td><span className="code">{member.member_code ?? "미발급"}</span></td><td><StatusBadge status={member.status} /></td><td><div className="member-risk-cell"><strong>{member.login_state === "ONLINE" ? "온라인" : member.login_state === "TRYING" ? "로그인 시도" : member.login_state === "FAILED" ? "실패 기록" : "오프라인"}</strong><span className={Number(member.duplicate_risk_score ?? 0) >= 60 ? "risk-high" : Number(member.duplicate_risk_score ?? 0) >= 30 ? "risk-mid" : "risk-low"}>중복위험 {Number(member.duplicate_risk_score ?? 0)}점</span>{Array.isArray(member.duplicate_risk_flags) && member.duplicate_risk_flags.length > 0 && <small>{member.duplicate_risk_flags.slice(0,2).join(" · ")}</small>}</div></td><td><RoleManager memberId={member.id} role={member.role} canManage={currentAdmin.role === "SUPER_ADMIN"} /></td><td className="muted">{formatDateTime(member.created_at)}</td><td><MemberActions memberId={member.id} status={member.status} memberCode={member.member_code} canManage={member.id !== currentAdmin.id && (member.role === "USER" || currentAdmin.role === "SUPER_ADMIN")} /></td></tr>;
    }) : <tr><td colSpan={9}><div className="empty">조건에 맞는 회원이 없습니다.</div></td></tr>}</tbody></table></div>
  </>;
}
