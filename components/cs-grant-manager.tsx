...
import { Coins, LoaderCircle, RotateCcw, Ticket, UserRoundCheck, WalletCards } from "lucide-react";
    ...
    <div className="stack-xl">
      <section className="panel panel-pad">
        <div className="section-head">
          <div>
            <h2 className="panel-title">CS 지급·회수 콘솔</h2>
            <p className="muted">승인된 일반 회원 1명에게만 추첨권과 포인트를 지급하거나 회수할 수 있습니다. 회수는 보유 수량 안에서만 처리됩니다.</p>
          </div>
      ...
      <section className="panel panel-pad">
        <div className="section-head">
          <div>
            ...
            <p className="muted">문의 보상, 운영 보정 등으로 추첨권을 추가합니다.</p>
          </div>
          <p className="muted">현재 보유 {currentTicketBalance.toLocaleString()}장</p>
        </div>
        <form className="form-grid mt-4" onSubmit={submitTickets}>
          ...
          <div className="table-actions">
            <button className="btn btn-primary" disabled={loading === "tickets" || !profileId || !drawId}>
      ...
      <section className="panel panel-pad">
        <div className="section-head">
          <div>
            ...
            <p className="muted">잘못 지급된 추첨권을 보유 수량 안에서 차감합니다. 회수 사유는 기록에 남습니다.</p>
          </div>
          <p className="muted">회수 가능 {currentTicketBalance.toLocaleString()}장</p>
        </div>
        <form className="form-grid mt-4" onSubmit={recoverTickets}>
          <label className="field-label">
            <span>회수 수량</span>
            <input className="input" type="number" min={1} max={Math.max(1, currentTicketBalance)} value={ticketRecoveryQuantity} onChange={(event) => setTicketRecoveryQuantity(Number(event.target.value || 1))} />
          </label>
          <label className="field-label">
            <span>회수 사유</span>
            <input className="input" value={ticketRecoveryReason} onChange={(event) => setTicketRecoveryReason(event.target.value)} placeholder="예: 잘못 지급" required />
          </label>
          <label className="field-label">
            <span>메모</span>
            <input className="input" value={ticketRecoveryMemo} onChange={(event) => setTicketRecoveryMemo(event.target.value)} placeholder="예: 문의번호, 처리자 메모" />
          </label>
          <div className="table-actions">
            <button className="btn btn-danger" disabled={loading === "recover-tickets" || !profileId || !drawId || currentTicketBalance < 1}>
      ...
      <section className="panel panel-pad">
        <div className="section-head">
          <div>
            ...
            <p className="muted">운영용 포인트/화폐를 회원에게 추가합니다.</p>
          </div>
          <p className="muted">현재 보유 {currentCurrencyBalance.toLocaleString()}</p>
        </div>
        <form className="form-grid mt-4" onSubmit={submitCurrency}>
          ...
          <div className="table-actions">
            <button className="btn btn-primary" disabled={loading === "currency" || !profileId || !currencyId}>
      ...
      <section className="panel panel-pad">
        <div className="section-head">
          <div>
            ...
            <p className="muted">잘못 지급된 포인트/화폐를 보유 수량 안에서 차감합니다.</p>
          </div>
          <p className="muted">회수 가능 {currentCurrencyBalance.toLocaleString()}</p>
        </div>
        <form className="form-grid mt-4" onSubmit={recoverCurrency}>
          <label className="field-label">
            <span>회수 수량</span>
            <input className="input" type="number" min={1} max={Math.max(1, currentCurrencyBalance)} value={currencyRecoveryAmount} onChange={(event) => setCurrencyRecoveryAmount(Number(event.target.value || 1))} />
          </label>
          <label className="field-label">
            <span>회수 사유</span>
            <input className="input" value={currencyRecoveryReason} onChange={(event) => setCurrencyRecoveryReason(event.target.value)} placeholder="예: 잘못 지급" required />
          </label>
          <label className="field-label">
            <span>메모</span>
            <input className="input" value={currencyRecoveryMemo} onChange={(event) => setCurrencyRecoveryMemo(event.target.value)} placeholder="예: 문의번호, 처리자 메모" />
          </label>
          <div className="table-actions">
            <button className="btn btn-danger" disabled={loading === "recover-currency" || !profileId || !currencyId || currentCurrencyBalance < 1}>
