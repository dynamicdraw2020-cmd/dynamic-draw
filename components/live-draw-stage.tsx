...
export function LiveDrawStage({ drawId, initialResult, draw }: { drawId?: string; initialResult?: DrawResult | null; draw?: Draw | null }) {
  ...
  return <section className="panel live-stage roulette-stage" aria-live="polite"><div className="live-top"><span className="live-status"><span className="live-dot" /><Radio size={14} /> LIVE EVENT</span>{!configured && <button className="btn btn-secondary btn-sm" onClick={runDemo} disabled={stage !== "idle" && stage !== "revealed"}><Eye size={14} /> 연출 미리보기</button>}</div>{stage === "glowing" && <div className="stage-glow roulette-glow optimized-glow" />}<div className="roulette-machine live-roulette-machine"><div className="roulette-pointer" /><div className={`roulette-wheel optimized live-roulette-wheel clean-live-wheel ${spinning || stage === "revealed" ? "is-spinning" : ""}`} style={{ background: gradient, "--spin-rotation": `${visualRotation}deg`, "--spin-duration": `${duration}ms`, "--segment-count": segments.length } as RouletteStyle}><div className="clean-wheel-center"><Trophy size={30} /></div></div></div><div className="live-result-strip" style={{ "--result-color": color } as RouletteStyle}>{stage === "revealed" && result ? <><Gift size={21} /><strong>{result.rewardName}</strong><span>{maskName(result.participantName ?? null)} · {maskMemberCode(result.memberCode ?? null)}</span></> : <><Dices size={21} /><strong>{stageLabel(stage)}</strong><span></span></>}</div><p className="stage-message">{stageLabel(stage)}</p></section>;
}
