...
export function LiveDrawStage({ drawId, initialResult, draw }: { drawId?: string; initialResult?: DrawResult | null; draw?: Draw | null }) {
  ...
  function runDemo() { beginAnimation(); timers.current.push(setTimeout(() => reveal({ rewardName: Math.random() > 0.72 ? "Dynamic" : "찢어진 입장권", rewardColor: "#fbbf24", participantName: "데모 회원", memberCode: "DD-2026-000001" }), 4000)); }
  const color = result?.rewardColor ?? "#f6c453";
  const spinning = stage === "spinning" || stage === "glowing" || stage === "slowing";
  const duration = draw?.animation_ms ?? 4000;

  return <section className="panel live-stage roulette-stage" aria-live="polite"><div className="live-top"><span className="live-status"><span className="live-dot" /><Radio size={14} /> LIVE ROULETTE v1.2</span>{!configured && <button className="btn btn-secondary btn-sm" onClick={runDemo} disabled={stage !== "idle" && stage !== "revealed"}><Eye size={14} /> 연출 미리보기</button>}</div>{stage === "glowing" && <div className="stage-glow roulette-glow optimized-glow" />}<div className="roulette-machine live-roulette-machine"><div className="roulette-pointer" /><div className={`roulette-wheel optimized live-roulette-wheel ${spinning || stage === "revealed" ? "is-spinning" : ""}`} style={{ background: gradient, "--spin-rotation": `${visualRotation}deg`, "--spin-duration": `${duration}ms`, "--segment-count": segments.length } as RouletteStyle}>{segments.slice(0, 8).map((reward, index) => { const angle = (360 / Math.max(segments.length, 1)) * index; return <span key={reward.id} style={{ transform: `rotate(${angle}deg) translateY(-42%) rotate(-${angle}deg)` }}>{reward.name}</span>; })}</div><div className="roulette-center"><Trophy size={34} /><span>Dynamic</span></div></div><div className="live-result-strip" style={{ "--result-color": color } as RouletteStyle}>{stage === "revealed" && result ? <><Gift size={21} /><strong>{result.rewardName}</strong><span>{maskName(result.participantName ?? null)} · {maskMemberCode(result.memberCode ?? null)}</span></> : <><Dices size={21} /><strong>{stageLabel(stage)}</strong><span>결과는 서버에서 먼저 결정됩니다. 룰렛 칸은 모두 같은 크기입니다.</span></>}</div><p className="stage-message">{stageLabel(stage)}</p></section>;
}
