...
function describeRewardConfig(data: AdminRewardSystemData, reward: Record<string, unknown>) {
  const type = String(reward.type ?? reward.reward_type ?? reward.rewardType ?? "").toUpperCase();
  const amount = Math.max(1, Number(reward.amount ?? reward.rewardAmount ?? 1) || 1).toLocaleString();
  const label = typeof reward.label === "string" && reward.label.trim() ? reward.label.trim() : "";
  if (label) return `${label} ${amount}`;
  if (type === "RANDOM_BOX") return `${data.boxes.find((box) => box.id === (reward.boxId ?? reward.random_box_id))?.name ?? "랜덤박스"} ${amount}개`;
  if (type === "TICKET") return `${data.draws.find((draw) => draw.id === (reward.drawId ?? reward.draw_id))?.name ?? "뽑기"} 추첨권 ${amount}장`;
  if (type === "CURRENCY") return `${data.currencies.find((currency) => currency.id === (reward.currencyId ?? reward.currency_id))?.name ?? "화폐"} ${amount}`;
  if (type === "ITEM") return `${data.rewards.find((item) => item.id === (reward.rewardId ?? reward.reward_id))?.name ?? "상품"} ${amount}개`;
  if (type === "EXP") return `경험치 ${amount}`;
  return `보상 ${amount}`;
}

function rewardConfigSummary(data: AdminRewardSystemData, rewards: unknown) {
  const rows = Array.isArray(rewards) ? rewards : [];
  if (!rows.length) return "보상 없음";
  return rows.map((row) => describeRewardConfig(data, row as Record<string, unknown>)).join(" · ");
}
...
export function RewardSystemManager({ data }: { data: AdminRewardSystemData }) {
  ...
  const sections = [
    { key: "referral", label: "추천·가입 보상", help: "회원가입 승인 보상과 추천인/추천받은 회원 보상을 설정합니다." },
  ...
  return <div className="grid gap-3">
    ...
    <section className="panel panel-pad">
      <h2 className="panel-title">추천·보상 관리</h2>
      <p className="panel-description mt-1">기능별로 나눠서 관리할 수 있도록 보상 설정을 세부 카테고리로 정리했습니다.</p>
      <div className="admin-section-tabs mt-3" role="tablist" aria-label="추천 보상 설정 카테고리">
        {sections.map((section) => <button key={section.key} type="button" role="tab" aria-selected={activeSection === section.key} className={`admin-section-tab${activeSection === section.key ? " active" : ""}`} onClick={() => setActiveSection(section.key)}>{section.label}</button>)}
      </div>
      <div className="admin-section-help">{sections.find((section) => section.key === activeSection)?.help}</div>
    </section>
    ...
    <section className="panel panel-pad"><h2 className="panel-title">출석 규칙 목록</h2><div className="table-wrap mt-3"><table className="table"><thead><tr><th>규칙</th><th>조건</th><th>지급 보상</th><th>관리</th></tr></thead><tbody>{data.attendanceRules.length ? data.attendanceRules.map((rule) => <tr key={rule.id}><td>{rule.name}</td><td>{rule.rule_type} · {rule.required_count}일</td><td>{rewardConfigSummary(data, rule.rewards)}</td><td><button className="btn btn-danger btn-sm" type="button" onClick={() => action({ action: "delete-attendance-rule", ruleId: rule.id }, "출석 규칙을 삭제했습니다.")}>삭제</button></td></tr>) : <tr><td colSpan={4}><div className="empty">출석 규칙이 없습니다.</div></td></tr>}</tbody></table></div></section>
    </>}
