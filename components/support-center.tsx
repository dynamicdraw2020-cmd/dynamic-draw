...
type Ticket = { id: string; profile_id?: string | null; category: string; title: string; body: string; status: string; admin_reply: string | null; internal_memo?: string | null; attachments?: Attachment[] | null; created_at: string; updated_at: string; profiles?: ProfileLite | null };

const statusLabels: Record<string, string> = { OPEN: "신규", ANSWERED: "진행중", CLOSED: "닫힘" };
const statusOptions = [
  { value: "ALL", label: "전체" },
  { value: "OPEN", label: "신규" },
...
function userLabel(ticket: Ticket) {
  ...
  return profile?.display_name || profile?.username || ticket.profile_id || "회원";
}

function userCode(ticket: Ticket) {
  const profile = ticket.profiles;
  return profile?.member_code || profile?.username || ticket.profile_id || "-";
}
    ...
    <div className="support-modal-card panel panel-pad">
      <div className="support-modal-head">
        <div>
          ...
          <h2>{ticket.title}</h2>
          <p>{ticket.category} · {new Date(ticket.created_at).toLocaleString("ko-KR")}</p>
        </div>
        <button className="btn btn-secondary btn-sm" type="button" onClick={onClose}><X size={16} /> 닫기</button>
      </div>
      {admin && <div className="support-detail-grid"><strong>문의자</strong><span>{userLabel(ticket)}</span><strong>유저 ID</strong><span>{userCode(ticket)}</span><strong>Profile ID</strong><span>{ticket.profile_id ?? "-"}</span></div>}
      ...
      {admin && ticket.internal_memo && <div className="support-detail-block"><strong>내부 메모</strong><p>{ticket.internal_memo}</p></div>}
    </div>
  </div>;
}
...
export function SupportCenter({ tickets }: { tickets: Ticket[] }) {
  ...
  return <div className="grid gap-3 support-mobile-stack">
    ...
    <section className="panel panel-pad support-list-card"><div className="support-list-head"><h2 className="panel-title">내 문의 내역</h2><select className="select support-filter-select" value={filter} onChange={(event) => setFilter(event.target.value)}>{statusOptions.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</select></div><div className="support-ticket-list mt-3">{filteredTickets.length ? filteredTickets.map((ticket) => <article className="support-ticket-card" key={ticket.id}><div className="support-ticket-main"><span className={statusClass(ticket.status)}>{statusLabel(ticket.status)}</span><strong>{ticket.title}</strong><p>{ticket.category} · {ticket.body.length > 80 ? `${ticket.body.slice(0, 80)}…` : ticket.body}</p><time>{new Date(ticket.created_at).toLocaleString("ko-KR")}</time></div><button className="btn btn-secondary btn-sm" type="button" onClick={() => setSelected(ticket)}>자세히 보기</button></article>) : <div className="empty">문의 내역이 없습니다.</div>}</div></section>
...
export function AdminSupportManager({ tickets }: { tickets: Array<Ticket & { profiles?: ProfileLite | null }> }) {
  ...
  return <section className="panel panel-pad admin-support-panel">
    ...
    <div className="support-admin-filters">
      <select className="select" value={statusFilter} onChange={(event) => setStatusFilter(event.target.value)}>{statusOptions.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</select>
      <div className="support-search"><Search size={16} /><input className="input" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="유저 ID, 닉네임, 제목 검색" /></div>
    </div>
    <div className="support-admin-list mt-3">
      {filteredTickets.length ? filteredTickets.map((ticket) => <article className="support-admin-card" key={ticket.id}>
        <div className="support-admin-summary"><span className={statusClass(ticket.status)}>{statusLabel(ticket.status)}</span><strong>{ticket.title}</strong><p>{userLabel(ticket)} · {userCode(ticket)} · {ticket.category}</p><time>{new Date(ticket.created_at).toLocaleString("ko-KR")}</time></div>
        <p className="support-body-preview">{ticket.body.length > 120 ? `${ticket.body.slice(0, 120)}…` : ticket.body}</p>
        <AttachmentPreview items={ticket.attachments} />
      ...
      </article>) : <div className="empty">조건에 맞는 문의가 없습니다.</div>}
    </div>
