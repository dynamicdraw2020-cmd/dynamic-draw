...
export function TicketGrantManager({ draws, members, balances, currencies, currencyBalances, exchangeRates }: { draws: Draw[]; members: Profile[]; balances: AdminTicketBalance[]; currencies: VirtualCurrency[]; currencyBalances: AdminCurrencyBalance[]; exchangeRates: Array<TicketExchangeRate & { draw_name?: string; currency_name?: string; currency_symbol?: string }> }) {
  ...
  const filteredMembers = useMemo(() => {
    ...
    return approvedMembers.filter((member) => [member.display_name, displayLoginId(member), member.member_code ?? "", member.role].some((value) => String(value).toLowerCase().includes(q)));
  ...
  async function submitTickets(event: FormEvent<HTMLFormElement>) {
    ...
    if (targetMode === "ALL" && !window.confirm(`승인된 계정 ${approvedMembers.length.toLocaleString()}명에게 각각 ${quantity.toLocaleString()}장씩 지급할까요?`)) return;
    setLoading("tickets");
    try {
  ...
  async function toggleCurrency(currency: VirtualCurrency) {
    ...
    try { await jsonRequest(`/api/admin/currencies/${currency.id}`, { isActive: !currency.is_active }, "PATCH"); router.refresh(); }
    catch (error) { window.alert((error as Error).message); } finally { setLoading(null); }
  }
  ...
  async function editRate(rate: TicketExchangeRate & { draw_name?: string; currency_name?: string; currency_symbol?: string }) {
    ...
    try { await jsonRequest(`/api/admin/ticket-exchange-rates/${rate.id}`, { currencyCost: cost, ticketQuantity: tickets }, "PATCH"); router.refresh(); }
    catch (error) { window.alert((error as Error).message); } finally { setLoading(null); }
  }

  async function toggleRate(rate: TicketExchangeRate & { draw_name?: string; currency_name?: string; currency_symbol?: string }) {
    setLoading(`rate-${rate.id}`);
    try { await jsonRequest(`/api/admin/ticket-exchange-rates/${rate.id}`, { isActive: !rate.is_active }, "PATCH"); router.refresh(); }
    catch (error) { window.alert((error as Error).message); } finally { setLoading(null); }
  }

  async function deleteRate(rate: TicketExchangeRate & { draw_name?: string; currency_name?: string; currency_symbol?: string }) {
    if (!window.confirm(`${rate.draw_name ?? "뽑기"} 교환비를 삭제할까요?`)) return;
    setLoading(`rate-${rate.id}`);
    try { await jsonRequest(`/api/admin/ticket-exchange-rates/${rate.id}`, {}, "DELETE"); router.refresh(); }
    catch (error) { window.alert((error as Error).message); } finally { setLoading(null); }
  }
  ...
  return <div className="grid ticket-admin-grid">
    ...
    <div className="grid grid-2">
      <form className="panel panel-pad form-grid" onSubmit={submitTickets}>
        <div><div className="flex items-center gap-1"><Ticket size={19} className="text-gold" /><h2 className="panel-title mb-0">추첨권 지급</h2></div><p className="panel-description mt-1">개별 회원 또는 전체 승인 계정에게 뽑기별 추첨권을 지급합니다.</p></div>
        <div className="form-row"><div className="field"><label>지급 대상</label><select className="select" value={targetMode} onChange={(event) => setTargetMode(event.target.value as "ONE" | "ALL")}><option value="ONE">회원 1명</option><option value="ALL">전체 승인 계정</option></select></div><div className="field"><label htmlFor="ticket-draw">뽑기</label><select id="ticket-draw" className="select" value={drawId} onChange={(event) => setDrawId(event.target.value)} required>{activeDraws.map((draw) => <option key={draw.id} value={draw.id}>{draw.name} · {draw.status}</option>)}</select></div></div>
        {targetMode === "ONE" && <div className="field member-search-field"><label htmlFor="ticket-member-search">회원 검색</label><input id="ticket-member-search" className="input" value={memberSearch} onChange={(event) => setMemberSearch(event.target.value)} placeholder="닉네임, 아이디, 고유 ID 검색" /><select id="ticket-member" className="select mt-2" value={profileId} onChange={(event) => setProfileId(event.target.value)} required>{filteredMembers.map((member) => <option key={member.id} value={member.id}>{member.display_name} · {displayLoginId(member)} · {member.role} · {member.member_code ?? "고유 ID 없음"}</option>)}</select><span className="text-muted text-small mt-1">검색 결과 {filteredMembers.length.toLocaleString()}명</span></div>}
      ...
      <form className="panel panel-pad form-grid" onSubmit={submitCurrency}>
        ...
        <div className="form-row"><div className="field"><label>지급 대상</label><select className="select" value={currencyTargetMode} onChange={(event) => setCurrencyTargetMode(event.target.value as "ONE" | "ALL")}><option value="ONE">회원 1명</option><option value="ALL">전체 승인 계정</option></select></div><div className="field"><label htmlFor="currency-select">화폐</label><select id="currency-select" className="select" value={currencyId} onChange={(event) => setCurrencyId(event.target.value)} required>{activeCurrencies.map((currency) => <option key={currency.id} value={currency.id}>{currency.name} · {currency.symbol}</option>)}</select></div></div>
        {currencyTargetMode === "ONE" && <div className="field member-search-field"><label htmlFor="currency-member-search">회원 검색</label><input id="currency-member-search" className="input" value={memberSearch} onChange={(event) => setMemberSearch(event.target.value)} placeholder="닉네임, 아이디, 고유 ID 검색" /><select id="currency-member" className="select mt-2" value={profileId} onChange={(event) => setProfileId(event.target.value)} required>{filteredMembers.map((member) => <option key={member.id} value={member.id}>{member.display_name} · {displayLoginId(member)} · {member.role} · {member.member_code ?? "고유 ID 없음"}</option>)}</select><span className="text-muted text-small mt-1">검색 결과 {filteredMembers.length.toLocaleString()}명</span></div>}
