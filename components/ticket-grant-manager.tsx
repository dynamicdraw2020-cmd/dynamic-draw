  ...
  async function submitCurrency(event: FormEvent) {
    ...
    try {
      const body = await jsonRequest("/api/admin/currency-grants", {
        ...
        profileId: currencyTargetMode === "ALL" ? null : profileId,
  ...
  return (
    <div className="stack-xl">
      <section className="panel panel-pad">
        <div className="section-head">
          <div>
            ...
            <p className="muted">지급과 회수를 한 화면에서 처리합니다. 회수는 개별 회원의 현재 보유 수량 안에서만 가능합니다.</p>
          </div>
        </div>
      </section>
      ...
      <section className="panel panel-pad">
        <div className="section-head">
          <div>
            ...
            <p className="muted">개별 회원 또는 전체 승인 일반 회원에게 뽑기별 추첨권을 지급합니다.</p>
          </div>
          <p className="muted">현재 보유 {targetMode === "ONE" ? `${currentTicketBalance.toLocaleString()}장` : `${approvedMembers.length.toLocaleString()}명 대상`}</p>
        </div>
        <form className="form-grid mt-4" onSubmit={submitTickets}>
          ...
          <div className="table-actions">
            <button className="btn btn-primary" disabled={loading === "tickets" || !drawId || (targetMode === "ONE" && !profileId)}>
      ...
      <section className="panel panel-pad">
        <div className="section-head">
          <div>
            <h2 className="panel-title"><RotateCcw size={20} /> 추첨권 회수</h2>
            <p className="muted">잘못 지급된 추첨권을 회원별로 회수합니다. 전체 회수는 안전상 제공하지 않습니다.</p>
          </div>
          <p className="muted">회수 가능 {recoverTicketBalance.toLocaleString()}장</p>
        </div>
        <form className="form-grid mt-4" onSubmit={recoverTickets}>
          <label className="field-label">
            <span>뽑기</span>
          ...
          <div className="table-actions">
            <button className="btn btn-danger" disabled={loading === "recover-tickets" || !recoverDrawId || !recoverProfileId || recoverTicketBalance < 1}>
      ...
      <section className="panel panel-pad">
        <div className="section-head">
          <div>
            ...
            <p className="muted">운영용 포인트/화폐를 개별 또는 전체 승인 일반 회원에게 지급합니다.</p>
          </div>
        </div>
        <form className="form-grid mt-4" onSubmit={submitCurrency}>
          ...
          <div className="table-actions">
            <button className="btn btn-primary" disabled={loading === "currency" || !currencyId || (currencyTargetMode === "ONE" && !profileId)}>
      ...
      <section className="panel panel-pad">
        <div className="section-head">
          <div>
            <h2 className="panel-title"><RotateCcw size={20} /> 포인트 회수</h2>
            <p className="muted">잘못 지급된 포인트/화폐를 회원별로 회수합니다. 보유량보다 많이 차감할 수 없습니다.</p>
          </div>
          <p className="muted">회수 가능 {recoverCurrencyBalance.toLocaleString()}</p>
        </div>
        <form className="form-grid mt-4" onSubmit={recoverCurrency}>
          <label className="field-label">
            <span>포인트/화폐</span>
          ...
          <div className="table-actions">
            <button className="btn btn-danger" disabled={loading === "recover-currency" || !recoverCurrencyId || !recoverCurrencyProfileId || recoverCurrencyBalance < 1}>
      ...
      <section className="panel panel-pad">
        <div className="section-head">
          <div>
            <h2 className="panel-title"><Plus size={20} /> 화폐 설정</h2>
            <p className="muted">이벤트용 포인트의 이름과 표기를 관리합니다.</p>
          </div>
        </div>
        <form className="form-grid mt-4" onSubmit={createCurrency}>
          <label className="field-label">
            <span>화폐명</span>
            <input className="input" value={newCurrencyName} onChange={(event) => setNewCurrencyName(event.target.value)} />
          </label>
          <label className="field-label">
            <span>표기</span>
            <input className="input" value={newCurrencySymbol} onChange={(event) => setNewCurrencySymbol(event.target.value)} />
          </label>
          <label className="field-label">
            <span>관리 코드</span>
            <input className="input" value={newCurrencyCode} onChange={(event) => setNewCurrencyCode(event.target.value.toUpperCase())} placeholder="EVENT_POINT" />
          </label>
          <div className="table-actions">
            <button className="btn btn-primary" disabled={loading === "create-currency"}>{loading === "create-currency" ? <LoaderCircle size={16} className="spin" /> : <Plus size={16} />} 화폐 만들기</button>
          </div>
        </form>
        <div className="table-wrap mt-4">
          <table className="data-table">
            <thead><tr><th>화폐</th><th>코드</th><th>상태</th><th>관리</th></tr></thead>
            <tbody>
              {currencies.map((currency) => (
                <tr key={currency.id}>
                  <td>{currency.name} · {currency.symbol}</td>
                  <td>{currency.code}</td>
                  <td>{currency.is_active ? "사용" : "정지"}</td>
                  <td>
                    <div className="table-actions compact">
                      <button className="btn btn-secondary btn-sm" type="button" onClick={() => void toggleCurrency(currency)}>{currency.is_active ? "정지" : "복구"}</button>
                      <button className="btn btn-danger btn-sm" type="button" onClick={() => void deleteCurrency(currency)}>삭제</button>
                    </div>
                  </td>
                </tr>
              ))}
              {!currencies.length && <tr><td colSpan={4}>화폐가 없습니다.</td></tr>}
            </tbody>
          </table>
        </div>
      </section>
      ...
      <section className="panel panel-pad">
        <div className="section-head">
          <div>
            <h2 className="panel-title"><Repeat2 size={20} /> 화폐 → 추첨권 교환 설정</h2>
            <p className="muted">회원이 직접 뽑기 화면에서 교환할 비율입니다.</p>
          </div>
        </div>
        <form className="form-grid mt-4" onSubmit={createRate}>
          <label className="field-label">
            <span>대상 뽑기</span>
            <select className="select" value={rateDrawId} onChange={(event) => setRateDrawId(event.target.value)}>
              {activeDraws.map((draw) => <option key={draw.id} value={draw.id}>{draw.name} · {draw.status}</option>)}
            </select>
          </label>
          <label className="field-label">
            <span>사용 화폐</span>
            <select className="select" value={rateCurrencyId} onChange={(event) => setRateCurrencyId(event.target.value)}>
              {activeCurrencies.map((currency) => <option key={currency.id} value={currency.id}>{currency.name}</option>)}
            </select>
          </label>
          <label className="field-label">
            <span>차감 화폐</span>
            <input className="input" type="number" min={1} value={currencyCost} onChange={(event) => setCurrencyCost(Number(event.target.value || 1))} />
          </label>
          <label className="field-label">
            <span>지급 추첨권</span>
            <input className="input" type="number" min={1} value={ticketQuantity} onChange={(event) => setTicketQuantity(Number(event.target.value || 1))} />
          </label>
          <div className="table-actions">
            <button className="btn btn-primary" disabled={loading === "rate"}>{loading === "rate" ? <LoaderCircle size={16} className="spin" /> : <Repeat2 size={16} />} 교환 비율 만들기</button>
          </div>
        </form>
        <div className="table-wrap mt-4">
          <table className="data-table">
