...
export function UserRouletteDraw({ draws, tickets, currencies, exchangeRates, copy }: { draws: Draw[]; tickets: UserDrawTicket[]; currencies: UserCurrencyBalance[]; exchangeRates: UserTicketExchangeRate[]; copy?: { playHeroTitle?: string; playHeroDescription?: string; probabilityTitle?: string; probabilityDescription?: string } }) {
  ...
  return <div className="grid">
    <section className="panel panel-pad draw-play-hero"><div><span className="eyebrow"><Ticket size={14} /> DRAW / EXCHANGE</span><h1>{copy?.playHeroTitle ?? "내 추첨권으로 뽑기 & 교환하기"}</h1><p>{copy?.playHeroDescription ?? "룰렛 칸은 모두 같은 크기로 보여 확률을 유추할 수 없습니다. 실제 결과는 서버 확률로 먼저 결정됩니다."}</p></div><div className="ticket-summary"><Ticket size={22} /><strong>{totalTickets.toLocaleString()}장</strong><span>전체 보유 추첨권</span></div></section>
