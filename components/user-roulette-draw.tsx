...
export function UserRouletteDraw({ draws, tickets, currencies, exchangeRates }: { draws: Draw[]; tickets: UserDrawTicket[]; currencies: UserCurrencyBalance[]; exchangeRates: UserTicketExchangeRate[] }) {
  ...
  const allDraws = useMemo(() => {
    ...
    for (const draw of draws) if (draw.status === "ACTIVE" && !draw.deleted_at) map.set(draw.id, draw);
    for (const ticket of tickets) if (ticket.draw.status === "ACTIVE" && !ticket.draw.deleted_at) map.set(ticket.draw.id, ticket.draw);
    for (const rate of exchangeRates) if (rate.draw.status === "ACTIVE" && !rate.draw.deleted_at) map.set(rate.draw.id, rate.draw);
    return Array.from(map.values());
  }, [draws, tickets, exchangeRates]);
  const firstDrawId = tickets.find((ticket) => ticket.quantity > 0 && ticket.draw.status === "ACTIVE")?.draw.id ?? allDraws[0]?.id ?? "";
